...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'Dex <dex@example.com>'
        tag.kvlm[None] = b"A tag generated by dex, which won't ;et you customize the message!\n"
...
def object_find(repo, name, fmt=None, follow=True):
    ...
    if not sha:
        raise Exception(f"No such references {name}.")
...
def index_read (repo):
    ...
    assert version == 2, "dex only supports index file version 2"
...
argsp.add_argument("--verbose", action="store_true", help="Show everythong")
...
def check_ignore_absolute(rules, path):
    parent = os.path.dirname(path)
    for rulest in rules:
        result = check_ignore1(rulest, path)
        if result != None: 
            return result
    return False
